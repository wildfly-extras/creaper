...
def customDefinition = {
    'custom-modifiable-realm'(customAttrs) {
...
def existingCustomElement = elytronSubsystem.'security-realms'.'custom-modifiable-realm'.find { it.'@name' == atrName }
if (existingCustomElement && !atrReplaceExisting) {
    throw new IllegalStateException("CustomModifiableRealm with name $atrName already exists in configuration. Use different name.")
} else {
