...
def customDefinition = {
    'custom-principal-transformer'(customAttrs) {
...
def existingCustomElement = elytronSubsystem.'mappers'.'custom-principal-transformer'.find { it.'@name' == atrName }
if (existingCustomElement && !atrReplaceExisting) {
    throw new IllegalStateException("CustomPrincipalTransformer with name $atrName already exists in configuration. Use different name.")
} else {
