...
def customDefinition = {
    'custom-realm-mapper'(customAttrs) {
...
def existingCustomRealmMapper = elytronSubsystem.'mappers'.'custom-realm-mapper'.find { it.'@name' == atrName }
if (existingCustomRealmMapper && !atrReplaceExisting) {
    throw new IllegalStateException("CustomRealmMapper with name $atrName already exists in configuration. Use different name.")
} else {
    if (existingCustomRealmMapper) {
        existingCustomRealmMapper.replaceNode customDefinition
    } else {
