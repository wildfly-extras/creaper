...
public class ChangeRootLoggerOnlineTest {
    ...
    private static final Address TEST_ROOT_LOGGER_ADDRESS = Address.subsystem("logging").and("root-logger", ROOT_NAME);
    ...
    public void changeEverything() throws Exception {
        ChangeRootLogger changeRootLogger = Logging.rootLogger().change()
        ...
        assertEquals("level should be changed", LogLevel.OFF.value(),
        ...
        assertEquals(1, ops.readAttribute(TEST_ROOT_LOGGER_ADDRESS, "handlers").stringListValue().size());
    }
}
