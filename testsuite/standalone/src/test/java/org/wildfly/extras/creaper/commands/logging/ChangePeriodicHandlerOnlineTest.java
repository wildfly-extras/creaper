...
public class ChangePeriodicHandlerOnlineTest {
    ...
    public void changeHandler() throws Exception {
        AddPeriodicHandler addHandler = new AddPeriodicHandler.Builder(TEST_HANDLER_NAME, "server.log", ".yyyy")
                .level(Level.FINEST)
                .filter("match(\"filter*\")")
                .setAutoFlush(false)
                .setEnabled(false)
        ...
        ChangePeriodicHandler changeHandler = new ChangePeriodicHandler.Builder(TEST_HANDLER_NAME, "new-server.log", ".yyyy")
                .level(Level.WARN)
        ...
        result.assertSuccess();
        assertEquals("log file has not been changed", "new-server.log", result.get("result").get("path").asString());
